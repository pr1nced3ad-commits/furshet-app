...
// --- Остальной код, как в рабочей версии с аккордеоном ---
const cart = {};
function renderMenu() { /* ... код не меняется ... */ }
window.addToCart = function(id) { /* ... код не меняется ... */ }
window.removeFromCart = function(id) { /* ... код не меняется ... */ }
function updateCartDisplay() { /* ... код не меняется ... */ }

// --- ФИНАЛЬНЫЙ, ИСПРАВЛЕННЫЙ ОБРАБОТЧИК КНОПКИ ---
webApp.onEvent('mainButtonClicked', function() {
    ...
    // --- ИСПРАВЛЕНИЕ ЗДЕСЬ: Используем prompt для ручного ввода номера ---
    ...
    // Если номер введен, собираем все данные для отправки
    const orderData = { 
        cart: {}, 
        totalPrice: 0, 
        userInfo: webApp.initDataUnsafe.user,
        phoneNumber: phoneNumber // Добавляем введенный номер
    };
    ...
    // Отправляем полные данные на "Кухню"
    fetch(BACKEND_URL, { 
        method: 'POST', 
